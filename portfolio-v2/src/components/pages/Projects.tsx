...
export function Projects()
{
    return(
        <MouseParallaxChild className="grid grid-cols-3 gap-8 size-full p-8 pb-16">
            <div className="flex flex-col justify-center size-full col-span-2 gap-1">
                <div className="font-(family-name:--font-haas-grot-disp-55-roman) text-xl tracking-wide">PROJECTS</div>
                <div className="text-xl/9 font-(family-name:--font-haas-grot-disp-55-roman) overflow-y-scroll grid grid-cols-2 grid-rows-4 gap-2 w-full">
                {
                    mapArray(8).map((v)=>{
                        ...
                        else
                        {
                            ...
                            return(
                            <>
                                <div id={"project"+v} className="w-full h-full flex flex-row gap-4 p-2 border border-b-8 border-b-zinc-500 border-zinc-700 font-(family-name:--font-haas-grot-disp-65) bg-zinc-800">
                                ...
                                <Tooltip disableStyleInjection style={{backgroundColor: "rgb(0,0,0,0)", padding: 0}} anchorSelect={"#project"+v} float noArrow place="right" opacity={1}>
                                    <div className="flex flex-col w-96 bg-zinc-800/90">
                                        <div className="bg-[#623b74] font-(family-name:--font-haas-grot-disp-65) text-3xl p-4 pb-2">
                                            {p.name.toUpperCase()}
                                            <div className="text-xl/normal text-zinc-300 font-(family-name:--font-haas-grot-disp-55-roman) flex flex-row w-full justify-between">
                                                <div>{p.time}</div>
                                                <div>{p.rarity}</div>
                                            </div>
                                        </div>
